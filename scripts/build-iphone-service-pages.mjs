...
<body>
  <header class="site-header"><div class="container header-inner">
    <a class="brand" href="${locale.home}" aria-label="${escapeHtml(locale.homeLabel)}"><img src="/assets/logo.jpg" alt="TecPro99"><span>TEC<span>PRO</span>99</span></a>
    <nav aria-label="${escapeHtml(locale.navServices)}"><a href="#repairs">${escapeHtml(locale.navServices)}</a><a href="#models">${escapeHtml(locale.navModels)}</a><a href="#process">${escapeHtml(locale.navProcess)}</a><a href="#faq">${escapeHtml(locale.navFaq)}</a></nav>
    <div class="header-actions"><details class="language"><summary aria-label="${escapeHtml(`${locale.languageLabel}: ${locale.code}`)}"><i data-lucide="languages" aria-hidden="true"></i>${locale.code}</summary><div class="language-menu">${languageMenu}</div></details><a class="button button-primary" href="${requestUrl}">${escapeHtml(locale.request)}</a></div>
  </div></header>
  <main>
    <section class="hero"><img class="hero-media" src="/assets/service-screen.jpg" alt="${escapeHtml(locale.title.split('|')[0].trim())}" fetchpriority="high"><div class="container hero-inner"><p class="eyebrow">${escapeHtml(locale.eyebrow)}</p><h1>${escapeHtml(locale.h1)}</h1><p class="hero-copy">${escapeHtml(locale.heroCopy)}</p><div class="hero-actions"><a class="button button-primary" href="${requestUrl}"><i data-lucide="clipboard-list" aria-hidden="true"></i>${escapeHtml(locale.request)}</a><a class="button button-light" href="${shared.phoneHref}"><i data-lucide="phone" aria-hidden="true"></i>${escapeHtml(locale.call)}</a></div></div></section>
    ...
    <section class="area"><div class="container"><p class="eyebrow">${escapeHtml(locale.areaEyebrow)}</p><h2>${escapeHtml(locale.areaTitle)}</h2><p>${escapeHtml(locale.areaCopy)}</p><div class="area-actions"><a class="button button-primary" href="${shared.directions}" target="_blank" rel="noopener"><i data-lucide="navigation" aria-hidden="true"></i>${escapeHtml(locale.directions)}</a><a class="button button-light" href="${shared.phoneHref}"><i data-lucide="phone" aria-hidden="true"></i>${shared.phone}</a></div></div></section>
    ...
    <section class="final"><div class="container final-inner"><div><h2>${escapeHtml(locale.finalTitle)}</h2><p>${escapeHtml(locale.finalCopy)}</p></div><div class="final-actions"><a class="button button-light" href="${requestUrl}">${escapeHtml(locale.request)}</a><a class="button" href="${shared.phoneHref}">${escapeHtml(locale.call)}</a></div></div></section>
    <div class="disclaimer"><div class="container">${escapeHtml(locale.independent)}</div></div>
  </main>
  <footer><div class="container footer-inner"><div><strong>TecPro99</strong><p>${escapeHtml(locale.footer)}</p><p>${shared.address} · ${shared.phone}</p></div><div class="footer-links"><a href="${locale.home}">${escapeHtml(locale.homeLabel)}</a><a href="${shared.directions}" target="_blank" rel="noopener">${escapeHtml(locale.directions)}</a><a href="${shared.phoneHref}">${shared.phone}</a></div></div></footer>
